...
const series_popCultureClassics = document.getElementById(
  "carouselSeries__popCultureClassics"
);
...
async function loadMedia(
  ...
  if (!element) {
    console.error(
      `Elemento DOM para ${loadingMessage} (associado ao endpoint base '${apiEndpointBase}') não encontrado.`
    );
    return [];
  }
  ...
  try {
        ...
        console.log(
          `${loadingMessage
            ...
            .replace(
              "...",
              ""
            )} (${fetchedPagesCount} páginas) carregados do cache.`
        );
      } else {
        console.log(
          `Cache de ${loadingMessage
            .replace("Carregando ", "")
            .replace("...", "")} expirado ou incompleto. Buscando novas mídias.`
        );
      }
    }
    ...
    if (!isListCacheValid) {
      for (let page = 1; page <= pagesToLoad; page++) {
        try {
          ...
          console.log(
            `Fazendo requisição à API: https://api.themoviedb.org/3/${endpointWithPage}`
          );
          const response = await fetch(
            `https://api.themoviedb.org/3/${endpointWithPage}`
          );

          if (!response.ok) {
            // verifica se a resposta HTTP foi bem-sucedida (as duas requisições, no caso)
            throw new Error(
              `Erro HTTP! Status: ${response.status} ao carregar página ${page} de: ${apiEndpointBase}`
            );
          }
          ...
          console.error(
            `Erro ao buscar página ${page} para ${loadingMessage}:`,
            pageError
          );
      ...
      console.log(
        `Novas ${loadingMessage
          .replace("Carregando ", "")
          .replace("...", "")
          .toLowerCase()} (${pagesToLoad} páginas) salvas no cache.`
      );
    }
        ...
        if (!mediaDetails) {
          try {
            const mediaDetailsResponse = await fetch(
              `https://api.themoviedb.org/3/${mediaType}/${currentMedia.id}?api_key=${api_key}&language=en-US`
            );

            if (!mediaDetailsResponse.ok) {
              console.warn(
                `Aviso: Erro ao buscar detalhes para a ${mediaType} ID ${currentMedia.id}. Status: ${mediaDetailsResponse.status}`
              );
              mediaDetails = null;
            } else {
            ...
            console.error(
              `Erro ao buscar detalhes para a ${mediaType} ID ${currentMedia.id}:`,
              detailsError
            );
            mediaDetails = null;
          }
        ...
        if (posterImg) {
          posterImg.addEventListener("click", () => {
            ...
            const preloadedDetails = {
              ...
              number_of_seasons: mediaDetails
    ...
    console.error(
      `Erro ao carregar ${loadingMessage
        .replace("Carregando ", "")
        .replace("...", "")
        .toLowerCase()}:`,
      error
