...
const listItemTemplate = document.getElementById('listItemTemplate').content.firstElementChild;
const movieCardTemplate = document.getElementById('movieCardTemplate').content.firstElementChild; // Para carrossel
...
function saveAllUserListsToLocalStorage(allUserLists) {
    localStorage.setItem(LOCAL_STORAGE_USER_LISTS_KEY, JSON.stringify(allUserLists));
}
...
function createNewUserList(listName, initialMovieIds = []) {
    ...
    const nameExists = userLists.some(list => list.name.toLowerCase() === normalizedNewListName);
...
function updateExistingUserList(listId, newName, newMovieIds) {
    ...
    if (targetList) {
        ...
        const nameExists = userLists.some(list => list.id !== listId && list.name.toLowerCase() === normalizedNewName);
...
function initializeCarousel(carousel) {
    ...
    if (!carouselId) {
        console.warn(`Carrossel sem ID encontrado:`, carousel, `As setas e indicadores podem não funcionar corretamente.`);
        return;
    }
    ...
    const leftArrow = parentContainer.querySelector(`.arrow.carousel-arrow-left[data-carousel-id="${carouselId}"]`);
    const rightArrow = parentContainer.querySelector(`.arrow.carousel-arrow-right[data-carousel-id="${carouselId}"]`);
    const indicatorsContainer = parentContainer.querySelector(`.indicators[data-carousel-id="${carouselId}"]`);
 ...
 */
async function renderUserListMovies(carouselElement, movieIds, listId, mediaType = 'movie') {
    if (!carouselElement) {
        console.error("Elemento carrossel não encontrado para renderizar filmes da lista.");
        return;
    }
    ...
    if (!movieCardTemplate) {
        ...
        carouselElement.innerHTML = '<p style="color: red;">Erro: Template de mídia não encontrado.</p>';
        return;
    }
    ...
    const mediaPromises = movieIds.map(async (movieId) => {
        ...
        if (!mediaDetails) {
            try {
                const response = await fetch(`https://api.themoviedb.org/3/${mediaType}/${movieId}?api_key=${api_key}&language=en-US`);
                if (!response.ok) {
                    console.warn(`Aviso: Erro ao buscar detalhes para ${mediaType} ID ${movieId}. Status: ${response.status}`);
                    return null;
                }
                ...
                console.error(`Erro ao buscar detalhes para ${mediaType} ID ${movieId}:`, error);
                return null;
            }
        }
        return mediaDetails;
    ...
    moviesData.forEach(movie => {
        if (movie) {
            ...
            if (posterImg && movie.poster_path) {
                ...
                posterImg.alt = `Pôster da ${mediaType === 'movie' ? 'filme' : 'série'} ${movie.title || movie.name}`;
            } else {
            ...
            if (posterImg) {
                posterImg.addEventListener('click', () => {
                    ...
                    try {
                        ...
                        console.log('Detalhes pré-carregados salvos no sessionStorage para ID:', movie.id);
                    } catch (e) {
        ...
        if (confirm(`Tem certeza que deseja remover "${movie.title || movie.name}" desta lista?`)) {
            ...
            if (targetList) {
                ...
                if (mediaCard) {
                    ...
                    console.warn('Elemento "mediaCard" não encontrado para remoção do DOM.');
                }
                ...
                if (carouselElement) {
                    ...
                    console.warn('Elemento "carouselElement" não encontrado para reinicialização do carrossel.');
                }

            } else {
                console.warn(`Lista com ID "${listId}" não encontrada para o usuário atual.`);
            }
        }
    });
} else {
    console.warn('Botão de remover filme não encontrado na mediaCard. Verifique o seletor ou a estrutura HTML.');
}
...
async function renderAllUserLists() {
    ...
    if (userLists.length === 0) {
        listsContainer.innerHTML = '<p>Você ainda não tem listas. Crie uma nova lista!</p>';
        return;
    }
    ...
    for (const list of userLists) {
        ...
        const leftArrow = listElement.querySelector('.arrowLeft-icon').closest('.arrow');
        const rightArrow = listElement.querySelector('.arrowRight-icon').closest('.arrow');
            ...
            const mediaPromises = listToEdit.films.map(async (movieId) => {
                ...
                if (!mediaDetails) {
                    try {
                        ...
                        mediaDetails = await response.json();
                        localStorage.setItem(mediaDetailsCacheKey, JSON.stringify({data: mediaDetails, timestamp: Date.now()}));
                    } catch (error) {
...
function removeMovieFromModal(movieId) {
    moviesInCurrentModalList = moviesInCurrentModalList.filter(movie => movie.id !== movieId);
    renderSelectedMoviesInModal();
}
...
function renderSelectedMoviesInModal() {
    ...
    moviesInCurrentModalList.forEach(movie => {
        ...
        li.innerHTML = `
            <img src="${movie.poster_path ? img_url + movie.poster_path : '../public/img/placeholder.png'}" alt="Poster" class="selected-movie-poster">
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    saveListBtn.addEventListener('click', () => {
        ...
        const movieIds = moviesInCurrentModalList.map(movie => movie.id.toString()); // IDs como string
...
async function searchMovies(query) {
    ...
    try {
        const response = await fetch(`https://api.themoviedb.org/3/search/multi?api_key=${api_key}&language=en-US&query=${encodeURIComponent(query)}`);
        if (!response.ok) {
        ...
        const filteredResults = data.results.filter(
            media => (media.media_type === 'movie' || media.media_type === 'tv') && media.poster_path
        );
        ...
        filteredResults.forEach(media => {
            ...
            title.textContent = `${media.title || media.name} (${(media.release_date || media.first_air_date || '').substring(0, 4)})`;
        ...
        searchResults.innerHTML = '<p style="color: red;">Erro ao pesquisar. Tente novamente.</p>';
    }
}